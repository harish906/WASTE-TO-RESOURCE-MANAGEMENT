...
<html>
<head>
 <title>Stockofwaste database</title>
...
$conn = mysqli_connect("localhost", "root", "", "registration");
  ...
  $sql = "SELECT typewast, netweight,dumplocation FROM impwast";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["typewast"]. "</td><td>" . $row["netweight"] . "</td><td>". $row["dumplocation"]."</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
