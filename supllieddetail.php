...
$conn = mysqli_connect("localhost", "root", "", "registration");
  ...
  $sql = "SELECT eventname, date, time, location, unitpower FROM eventsupply";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["eventname"]. "</td><td>" . $row["date"] . "</td><td>". $row["time"]. "</td><td>". $row["location"] . "</td><td>". $row["unitpower"] . "</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
...
$conn = mysqli_connect("localhost", "root", "", "registration");
  ...
  $sql = "SELECT suplace, datee, timee, locatione, unitpowere FROM pubsupply";
  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {
    echo "<tr><td>" . $row["suplace"]. "</td><td>" . $row["datee"] . "</td><td>". $row["timee"]. "</td><td>". $row["locatione"] . "</td><td>". $row["unitpowere"] . "</td></tr>";
}
echo "</table>";
} else { echo "0 results"; }
$conn->close();
